...
configure_notifications() {
    ...
    mkdir -p /etc/xdc-node
    mkdir -p /var/lib/xdc-node
    mkdir -p /var/log/xdc-node
    
    # Copy notification library
    mkdir -p /opt/xdc-node/scripts/lib
    if [[ -f "$SCRIPT_DIR/scripts/lib/notify.sh" ]]; then
        cp "$SCRIPT_DIR/scripts/lib/notify.sh" /opt/xdc-node/scripts/lib/
        chmod +x /opt/xdc-node/scripts/lib/notify.sh
        log "✓ Notification library installed"
    fi
    
    # Copy email templates
    if [[ -d "$SCRIPT_DIR/templates/email" ]]; then
        mkdir -p /opt/xdc-node/templates/email
        cp -r "$SCRIPT_DIR/templates/email/"* /opt/xdc-node/templates/email/
        log "✓ Email templates installed"
    fi
    
    # Copy notify-test.sh
    if [[ -f "$SCRIPT_DIR/scripts/notify-test.sh" ]]; then
        cp "$SCRIPT_DIR/scripts/notify-test.sh" /opt/xdc-node/scripts/
        chmod +x /opt/xdc-node/scripts/notify-test.sh
        log "✓ Notification test script installed"
    fi
    
    # Check if config already exists
    if [[ -f "/etc/xdc-node/notify.conf" ]]; then
        log "✓ Notification config already exists"
        return 0
    fi
    ...
    if [[ "$NON_INTERACTIVE" != true ]]; then
        ...
        echo "=================================="
        echo ""
        echo "Configure how you want to receive alerts:"
        echo ""
        echo "1) XDC Gateway Platform (Recommended)"
        echo "   - Register at cloud.xdcrpc.com"
        echo "   - Get API key from dashboard"
        echo "   - Handles Telegram + Email"
        echo ""
        echo "2) Direct Telegram"
        echo "   - Use your own Telegram bot"
        echo "   - Create bot via @BotFather"
        echo ""
        echo "3) Email Only"
        echo "   - Direct SMTP configuration"
        echo ""
        ...
        local choice
        while true; do
            read -rp "Select notification method [1-4]: " choice
            case $choice in
                1) configure_platform_notifications; break ;;
                2) configure_telegram_notifications; break ;;
                3) configure_email_notifications; break ;;
                4) create_default_notify_config; break ;;
                *) echo "Invalid selection. Please choose 1-4." ;;
            esac
        done
    else
        # Non-interactive mode: check for env vars
        if [[ -n "${NOTIFY_PLATFORM_API_KEY:-}" ]]; then
            create_platform_config
        elif [[ -n "${NOTIFY_TELEGRAM_BOT_TOKEN:-}" && -n "${NOTIFY_TELEGRAM_CHAT_ID:-}" ]]; then
            create_telegram_config
        else
            create_default_notify_config
        fi
    fi
...
configure_platform_notifications() {
    echo ""
    read -rp "Enter your XDC Gateway API key: " api_key
    
    if [[ -z "$api_key" ]]; then
        warn "No API key provided, creating default config"
        create_default_notify_config
        return
    fi
    
    cat > /etc/xdc-node/notify.conf << EOF
...
EOF
    ...
    chmod 600 /etc/xdc-node/notify.conf
    log "✓ Platform notifications configured"
...
configure_telegram_notifications() {
    ...
    if [[ -z "$bot_token" || -z "$chat_id" ]]; then
        warn "Incomplete Telegram config, creating default config"
        create_default_notify_config
        return
    fi
...
configure_email_notifications() {
    echo ""
    read -rp "Enter recipient email address: " email_to
    read -rp "Enter SMTP host (e.g., smtp.gmail.com): " smtp_host
    read -rp "Enter SMTP port (e.g., 587): " smtp_port
    read -rp "Enter SMTP username: " smtp_user
    read -rsp "Enter SMTP password: " smtp_pass
    echo ""
    
    if [[ -z "$email_to" ]]; then
        warn "No email provided, creating default config"
        create_default_notify_config
        return
    fi
...
NOTIFY_EMAIL_SMTP_HOST="${smtp_host:-smtp.gmail.com}"
NOTIFY_EMAIL_SMTP_PORT="${smtp_port:-587}"
NOTIFY_EMAIL_SMTP_USER="$smtp_user"
NOTIFY_EMAIL_SMTP_PASS="$smtp_pass"
...
create_default_notify_config() {
    if [[ -f "$SCRIPT_DIR/configs/notify.conf.template" ]]; then
        cp "$SCRIPT_DIR/configs/notify.conf.template" /etc/xdc-node/notify.conf
    else
        cat > /etc/xdc-node/notify.conf << EOF
# XDC Node Notification Configuration
# Edit this file to configure notifications

# Notification channels (platform,telegram,email)
NOTIFY_CHANNELS="platform"
...
EOF
    ...
    chmod 600 /etc/xdc-node/notify.conf
    log "✓ Default notification config created"
    log "  Edit /etc/xdc-node/notify.conf to configure notifications"
}
...
test_notifications() {
    echo ""
    read -rp "Test notifications now? [Y/n]: " test_choice
    if [[ ! "$test_choice" =~ ^[Nn]$ ]]; then
        log "Testing notification configuration..."
        if [[ -f "/opt/xdc-node/scripts/notify-test.sh" ]]; then
            /opt/xdc-node/scripts/notify-test.sh --channels "$NOTIFY_CHANNELS" || true
        fi
    fi
}
